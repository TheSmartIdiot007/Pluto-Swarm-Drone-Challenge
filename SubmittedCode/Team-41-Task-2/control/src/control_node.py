...
class control :
    def __init__ (self) :
        ...
        self.slider_x = threading.Thread(target=start_slider, args=('X', self))
        self.slider_y = threading.Thread(target=start_slider, args=('Y', self))
        self.slider_z = threading.Thread(target=start_slider, args=('Z', self))
        self.slider_x.start()
        self.slider_y.start()
        self.slider_z.start()

    def loadPID_gains(self):
        self.update_K(np.load('Xsaved.npy'), 'X')
        self.update_K(np.load('Ysaved.npy'), 'Y')
        self.update_K(np.load('Zsaved.npy'), 'Z')
    ...
    def feedback(self) :
        ...
        if id in self.vision.X.keys():

            # position feedback from CV
            feedback_x = self.vision.X[id][0] / 100
            feedback_y = self.vision.Y[id][0] / 100
            feedback_z = 2.6132 -0.3 - (self.vision.Z[id][0]/100) # Transforming the z-coordinate of the camera to the ground frame
    ...
    def setpoint(self, desired_point, stay=False, time_interval = 10, deadband_width = 0.1):
        ...
        while (self.distance >= deadband_width or stay) :
            ...
            zero_throttle_value = 1600
            throt_value = int(zero_throttle_value + self.throttle_output)
            self.rcThrottle = self.clip(throt_value, 2199, 1500)
            print(self.rcThrottle)

            self.bridge.send_data(int(self.rcRoll), int(self.rcPitch), int(1491), int(self.rcThrottle))
...
class task2: 
    ...
    def hover(self, z):
        ...
        self.controller.bridge.arm()
        sleep(2)
        self.controller.setpoint([0,0,z], stay=5, deadband_width=.05)
        self.controller.bridge.disarm()
        

if __name__ == '__main__' :
    X = np.array([0.84, -.75, -0.73, 0.8]) -0.15
    Y = np.array([-0.31, -0.31, 0.45, 0.41])
    Z = 0.5
    hover = False
    T2 = task2(X, Y, Z)
    if hover:
