...
class Rectangle:
    ...
    def generate_points(self):
        self.X1 = np.linspace(self.X[0],self.X[1],num=self.S1,endpoint=True)
        self.X2 = np.linspace(self.X[1],self.X[2],num=self.S2,endpoint=True)
        self.X3 = np.linspace(self.X[2],self.X[3],num=self.S3,endpoint=True)
        self.X4 = np.linspace(self.X[3],self.X[0],num=self.S4,endpoint=True)
        self.Y1 = np.linspace(self.Y[0],self.Y[1],num=self.S1,endpoint=True)
        self.Y2 = np.linspace(self.Y[1],self.Y[2],num=self.S2,endpoint=True)
        self.Y3 = np.linspace(self.Y[2],self.Y[3],num=self.S3,endpoint=True)
